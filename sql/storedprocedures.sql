...
BEGIN
    ...
    begin
        ...
        SET @DaysLeft
            = datediff(
                          day
                        , (
                              SELECT RETURNEDDATE FROM CARRENTAL WHERE RENTALID = @RENTID
                          )
                        , (
                              SELECT RENTENDDATE FROM CARRENTAL WHERE RENTALID = @RENTID
                          )
                      )
        return;
    end
    ...
    FROM CARRENTAL                    cr
        INNER JOIN vwCarInfoWithPrice vw
            ON cr.CARID = vw.[Car ID]
    WHERE cr.RENTALID = @RentID;

    --GET START DATE AND END DATE
    SELECT @StartDate = RENTSTARTDATE
         , @EndDate   = RENTENDDATE
    FROM CARRENTAL          cr
...
BEGIN
	...
	BEGIN TRY
		...
		SET @RentID =
		(
			SELECT RENTALID FROM REFUNDREQUESTS WHERE REQUESTID = @RequestID
		)
		...
		SET @Total =
		(
			SELECT AMOUNT FROM CARRENTAL WHERE RENTALID = @RentID
		)
		...
		SET @CardID =
		(
			SELECT CARDID FROM CARRENTAL WHERE RENTALID = @RentID
		)
		...
		SET @CardNumber =
		(
			SELECT CARDNUMBER FROM VISACARD WHERE CARDID = @CardID
		)
...
BEGIN
    ...
    BEGIN TRY
        ...
        set @HaveCard =
        (
            select DBO.fnCardExistForUserId(@CustID, @CardNumber)
        );
        set @Rented =
        (
            select isRented from CAR where CARID = @CarID
        )
        set @HaveEnough =
        (
            select DBO.fnCheckBalance(@CardNumber)
        );
        ...
        set @Total =
        (
            select DBO.fnTotalWithPrem(@CarID, @NbDays, @CustID)
        );
        ...
        set @CardID =
        (
            select CARDID from VISACARD where CARDNUMBER = @CardNumber
        )
...
BEGIN
    ...
    BEGIN
        ...
        DECLARE @HashedPassword VARCHAR(256);
        SELECT @HashedPassword = [PASSWORDHASH]
        FROM CUSTOMER
        WHERE EMAIL = @Email;
        ...
        BEGIN
            ...
            SET @UserId =
            (
                SELECT DISTINCT CUSTOMERID FROM CUSTOMER WHERE EMAIL = @Email
            );
